...
SYSTEM_PROMPT = (
    ...
    "- User messages may embed an [ATTACHED PLAN: ...] block containing a saved plan's "
    "inputs and output; treat it as the user's current plan. Answer ranking or why "
    "questions from its score_breakdown data. For change requests, call "
    "build_investment_plan starting from the attached inputs with the requested "
    "modifications. Saved snapshots are immutable, so present the result as a new plan "
    "and offer save_plan for a new snapshot when the user confirms.\n"
